...
def get_target_distribution(train_df, out_dir):
    ...
    distribution = (
        alt.Chart(train_df, title="There is a greater number of young abalones than old abalones in the training data")
        .mark_bar()
...
def get_correlation_map(train_df, out_dir):
    ...
    correlation = (
        alt.Chart(corr_df,
        title="Many features and the target (rings) are highly correlated")
        .mark_rect()
        .encode(
